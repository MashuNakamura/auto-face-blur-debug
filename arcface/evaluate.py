#!/usr/bin/env python3
"""
ARCFACE - BATCH IMAGE EVALUATION (FINAL)
---------------------------------------------------
Evaluasi ArcFace + YOLO dari folder testing/ dengan subfolder:
...
def get_embedding(img_bgr):
    ...
    try:
        ...
        h, w = rgb.shape[:2]
        scale = 224 / min(h, w) if min(h,w)<224 else 512/max(h,w) if max(h,w)>512 else 1
