#!/usr/bin/env python3
"""
ARCFACE - REALTIME WEBCAM TEST (GPU MODE ONLY)
---------------------------------------------------
Detektor    : YOLOv11n (GPU)
Vektorisasi : ArcFace (GPU via TensorFlow)
...
if len(tf_gpus) > 0:
    ...
    print("[! ] ERROR: TensorFlow GPU not found!")
    exit(1)
...
while True:
    ...
    info_str = f"[GPU] ArcFace | FPS: {fps:.1f} (Avg: {avg_fps:.1f}) | Cache: {len(embedding_cache)}"
    ...
    cv2.imshow("ArcFace - GPU Mode", display_frame)
...
# ============================================================
print("\n" + "=" * 60)
print("PERFORMANCE SUMMARY - ARCFACE GPU MODE")
print("=" * 60)
...
print("|---------|--------|---------|-----------|------------|--------------------|")
print(f"| ArcFace | GPU    | {avg_fps:.1f}    | {YOLO_SKIP_FRAMES: <9} | {EMBEDDING_SKIP_FRAMES:<10} | {cache_hit_rate:. 1f}%               |")
print("\n")