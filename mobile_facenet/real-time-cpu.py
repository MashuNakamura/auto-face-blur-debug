#!/usr/bin/env python3
"""
MOBILEFACENET - REALTIME WEBCAM TEST (CPU MODE ONLY)
---------------------------------------------------
Detektor    : YOLOv11n (CPU)
Vektorisasi : MobileFaceNet (CPU)
...
while True:
    ...
    info_str = f"[CPU] MobileFaceNet | FPS:  {fps:.1f} (Avg: {avg_fps:.1f}) | Cache: {len(embedding_cache)}"
    ...
    cv2.imshow("MobileFaceNet - CPU Mode", display_frame)
...
# ============================================================
print("\n" + "=" * 60)
print("PERFORMANCE SUMMARY - MOBILEFACENET CPU MODE")
print("=" * 60)
...
print("|---------------|--------|---------|-----------|------------|--------------------|")
print(f"| MobileFaceNet | CPU    | {avg_fps:.1f}    | {YOLO_SKIP_FRAMES:<9} | {EMBEDDING_SKIP_FRAMES:<10} | {cache_hit_rate:.1f}%               |")
print("\n")